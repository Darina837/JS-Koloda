...
function getCurrentPostComments ( postId ) {
        var res = []
    for (var id_of_comment in comments) {
      if (comments[id_of_comment].postId !== postId) continue 
        res.push({
          author: users[comments[id_of_comment].author].name,
          text: comments[id_of_comment].text 
        })
    }
